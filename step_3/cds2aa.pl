...
die "perl $0 [cds fas] [NUC | Invertebrate MT] [output file]\n Lite version of translation, translate based on phase=0, no gap will be removed\n" unless (@ARGV == 3);
	...
	for(my $c=0; $c<$len; $c+=3){
		...
		if($ARGV[1] eq "NUC"){
			if($CODE_NUC{$codon}){
				...
				$aa .= "X";
				print "$name: Unknown codon is found: $codon\n";
			}
		}elsif($ARGV[1] eq "MT"){
			if($CODE_MT{$codon}){
				$aa .= $CODE_MT{$codon};
